...
def extract_midi(input_bytes, bp_model, noise_quartiles, signal_quartiles, temp_dir='./temps'):
    """Extracts MIDI from raw PyAudio bytes and returns serialized MIDI info.

    Currently dispatches to extract_midi_old_implementation (Basic Pitch).
    ...
    Args:
        ...
        noise_quartiles (tuple): (25th, 50th, 75th) RMS percentiles from noise calibration.
        signal_quartiles (tuple): (25th, 50th, 75th) RMS percentiles from signal calibration.
        temp_dir (str): Directory for temporary WAV/MIDI files. Defaults to './temps'.

    Returns:
        dict: Contains 'ticks_per_beat', 'messages', and 'is_empty'.
    """
...
def extract_midi_old_implementation(input_bytes, bp_model, noise_quartiles, signal_quartiles, temp_dir='./temps'):
    """Basic Pitch-based MIDI extraction implementation (currently active).

    Denoises the audio, writes a temp WAV, runs Basic Pitch AMT, serializes
    the result, then deletes all temporary files.
